                ...
                ([FromBody] LoginRequest login, [FromQuery] bool? useCookies, [FromQuery] bool? useSessionCookies, [FromServices] IServiceProvider sp) =>
                {
                    ...
                    if (!result.Succeeded)
                    {
                        ...
                        if (user is not null)
                        {
                            ...
                            {
                                var emailConfirmationLink = await userManager.GenerateEmailConfirmationTokenAsync(user);
                                emailConfirmationLink = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(emailConfirmationLink));
                               
                                Dictionary<string, object> info = new Dictionary<string, object>();

                                info.Add("EmailConfirmationLink", emailConfirmationLink);
                                info.Add("IsEmailConfirmed", false);

                                return TypedResults.Problem("You should verify you email first.",
                                    statusCode: StatusCodes.Status403Forbidden, extensions: info);
                            }
                        }
